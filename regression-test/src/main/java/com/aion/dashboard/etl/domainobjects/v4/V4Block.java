...
import com.aion.dashboard.etl.domainobjects.v5.V5Block;

import java.math.BigDecimal;
import java.math.BigInteger;
import java.util.Objects;

public class V4Block extends AbstractBlock {
    ...
    public boolean compare(AbstractBlock block) {
        if(block instanceof V5Block) {
            V5Block v5Block = (V5Block) block;

            String[] v5TransactionHashes = v5Block.getTransactionHashes().replace("[", "").replace("]", "").trim().split(",");
            for (String v5TransactionHash : v5TransactionHashes) {
                if(!transactionList.contains(v5TransactionHash)) {
                    return false;
                }
            }

            return super.compare(block) &&
                    size == v5Block.getBlockSize() &&
                    nrgReward.doubleValue() == v5Block.getNrgReward().doubleValue() &&
                    Long.parseLong(difficulty, 16) == v5Block.getDifficulty() &&
                    Long.parseLong(totalDifficulty, 16) == v5Block.getTotalDifficulty();
    ...
    public static class BlockBuilder {
        ...
        public V4Block build() {
            V4Block block = new V4Block();
