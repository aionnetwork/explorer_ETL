...
public class ParserStateTest {
    ...
    public void readState(){
        ...
        for (var state: res)
            System.out.println(String.format("ID =[%d], Block_num=[%d], tx_num=[%d]", state.getId(), state.getBlockNumber().longValue()));
    }
    ...
    public void updateState(){
        ...
        ParserState test_State =  builder.id(1).blockNumber(BigInteger.valueOf(5)).build();

        List<ParserState> updates = List.of(
                builder.id(1).blockNumber(BigInteger.valueOf(5)).build(),
                builder.id(2).blockNumber(BigInteger.valueOf(5)).build(),
                builder.id(3).blockNumber(BigInteger.valueOf(5)).build()
        );
